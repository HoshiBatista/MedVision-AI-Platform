"""
Pneumonia Detection — Debug: run inference and visualise predictions.
...
TEST_IMAGES = ROOT / "ml" / "data" / "pneumonia_detection" / "test" / "images"
TEST_LABELS = ROOT / "ml" / "data" / "pneumonia_detection" / "test" / "labels"

CLASSES = ["Atypical", "Indeterminate", "Typical"]
COLORS  = [(220, 50, 50), (50, 200, 50), (50, 50, 220)]   # BGR
...
def load_gt_boxes(label_path: Path, img_w: int, img_h: int) -> list[tuple]:
    """Parse YOLO bbox label → list of (class_id, x1, y1, x2, y2)."""
    boxes = []
...
def draw_boxes(img: np.ndarray, boxes: list[tuple], labels: list[str], colors: list, show_conf: bool = False) -> np.ndarray:
    out = img.copy()
    for item in boxes:
        if show_conf:
...
def main() -> None:
    ...
    fig.suptitle("Pneumonia Detection — Debug\n(left: GT  |  right: Prediction)", fontsize=14)
    axes = axes.flatten()
    ...
    for i, (img_path, result) in enumerate(zip(imgs, results)):
        ...
        gt_img   = cv2.cvtColor(draw_boxes(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), gt_boxes, CLASSES, COLORS), cv2.COLOR_BGR2RGB)
        pred_img = cv2.cvtColor(draw_boxes(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), pred_boxes, CLASSES, COLORS, show_conf=True), cv2.COLOR_BGR2RGB)
    ...
    if args.save:
        out_dir = ROOT / "ml" / "runs" / "pneumonia_detection" / "debug"
