...
- [x] **Admin/Users UI** — `/admin` (admin-only): список пользователей, PATCH role/full_name/is_active, deactivate; `api/admin.ts` + пункт в sidebar
- [x] **Дашборд/история исследований** — `/history`: stats cards, вкладки jobs/studies, `GET /api/v1/results` (list) + `GET /api/v1/upload`; клик по job → `/analysis?job=`
- [x] **Настоящий DICOM-вьюер** (cornerstone.js) — `@cornerstonejs/core` + dicom-image-loader для `.dcm`; PNG/JPEG fallback; overlay heatmap/bboxes сохранён
...
auth refresh-токены, password reset, gateway rate-limiting, Helm/K8s и OpenTelemetry/Jaeger — сделаны.
На GPU-хосте стоит один раз проверить `make up-gpu` (Triton + ollama на видеокарте) и подтвердить
реальный реф модели OpenBioLLM-8B GGUF. Для трассировки: `make up-monitoring` → Jaeger UI на :16686.
