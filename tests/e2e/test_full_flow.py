...
-> EigenCAM heatmap (served through the gateway) -> clinical report generated by
a local Ollama LLM (OpenBioLLM-8B). No GPU/Triton needed.
"""
...
RESULT_TIMEOUT = 120
# The Ollama model is pulled lazily in the background at startup (the container
# is "healthy" before the model is ready), and a fresh CI run re-downloads the
# weights, so the readiness wait is generous; CPU generation of one report is
# then bounded too.
...
def _wait_report_model_ready() -> None:
    """Block until report_service reports its Ollama model is pulled and ready."""
    ...
    while time.time() < deadline:
        try:
            ...
            last = str(exc)
        time.sleep(5)
    pytest.fail(f"Ollama model not ready within {REPORT_READY_TIMEOUT}s (last: {last})")
...
def test_pipeline_upload_analyze_results(client: httpx.Client, auth_headers: dict):
    ...
    # 4. Generate a clinical report from the findings via local Ollama
    #    (OpenBioLLM-8B, CPU). The model is pulled lazily, so wait for
    #    report_service to report it ready first.
    ...
    # 5. Poll until Ollama finishes generating the report.
